...
例如一 10 進位數字 3456，第二位(最右邊為第一位)數字為 5，則其代表的數值為 5* 10<sup>(2-1)</sup> = 5* 10<sup>1</sup>。一 16 進位數字 fe，第二位數字為 e，則其代表的數值為 15* 16<sup>(2-1)</sup> = 15* 16<sup>1</sup>。一 10 進位數字 001101101，第四位數字為 1，則其代表的數值為 1* 2<sup>(4-1)</sup> = 1* 2<sup>3</sup>。
...
a = a << 3;            //0x01000000 = 64
unsigned char b = 127; //0x11111111
b = b >> 3;            //0x01111111 = 63
//可以想成小數點被直接捨去。
```
...
#### 5. 乘法
因為電腦中只有 0 與 1，所以如果要使用位移運算，那麼我們必須將乘數拆成 2 的正整數次。例如 3* 7 = 3* 2<sup>2</sup> + 3* 2<sup>1</sup>+ 3* 2<sup>0</sup> = 3 << 2 + 3 << 1 + 3，7 = 111<sub>2</sub>，所以當乘數的末位 bit 為 1 時就做一次加法，直到乘數為 0
```C
